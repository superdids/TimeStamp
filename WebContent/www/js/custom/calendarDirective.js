...
app.controller('TimeStampCalendarCtrl', function($scope, $log, $timeout, $attrs, $locale, moment, calendarTitle, calendarHelper) {
	var vm = this;
	...
	calendarHelper.loadTemplates().then(function() {
		...
		$scope.$watchGroup([
			...
			if (!eventsWatched) {
				...
				$scope.$watch('vm.users', refreshCalendar, true); //this will call refreshCalendar when the watcher starts (i.e. now)
			} else {
				refreshCalendar();
			}
		});
...
.directive('timeStampCalendar', function(calendarConfig) {

	return {
		templateUrl: calendarConfig.templates.timeStampCalendar,
		restrict: 'E',
		scope: {
			users: '=',
			view: '=',
		...
		controller: 'TimeStampCalendarCtrl as vm',
